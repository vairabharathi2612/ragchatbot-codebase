    ...
    def _format_results(self, results: SearchResults) -> str:
        ...
        for doc, meta in zip(results.documents, results.metadata):
            ...
            if label not in seen_sources:
                ...
                # Look up lesson link and encode it with the label
                link = None
                if lesson_num is not None:
                    link = self.store.get_lesson_link(course_title, lesson_num)
                if link:
                    sources.append(f"{label}||{link}")
                else:
...
class CourseOutlineTool(Tool):
    ...
    def get_tool_definition(self) -> Dict[str, Any]:
        return {
            ...
            "description": "Get the full outline of a course: title, link, and every lesson number and title. Use this for any outline, syllabus, or lesson-list query.",
            "input_schema": {
                "type": "object",
                "properties": {
                    "course_title": {
                        "type": "string",
                        "description": "Course title to look up (partial matches work, e.g. 'MCP', 'Chroma')"
                    }
                },
